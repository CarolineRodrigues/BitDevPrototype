...
class ComponentCompiler {
  constructor(pubsub, workspace, component, compilerInstance, compilerId, logger, dists = [], compileErrors = []) {
    this.pubsub = pubsub;
  ...
  async compile(noThrow = true, options) {
    ...
    let dataToPersist;
    const deleteDistDir = (_options$deleteDistDi = options.deleteDistDir) !== null && _options$deleteDistDi !== void 0 ? _options$deleteDistDi : this.compilerInstance.deleteDistDir; // delete dist folder before transpilation (because some compilers (like ngPackagr) can generate files there during the compilation process)
...
class WorkspaceCompiler {
  constructor(workspace, envs, pubsub, aspectLoader, ui, logger) {
    this.workspace = workspace;
    this.envs = envs;
    this.pubsub = pubsub;

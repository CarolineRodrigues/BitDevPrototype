...
class WorkspaceManifestFactory {
  ...
  async getEnvsPeersPolicy(components) {
    const foundEnvs = await Promise.all(components.map(component => this.dependencyResolver.getComponentEnvPolicy(component)));
    const peersPolicies = foundEnvs.map(policy => policy.peersAutoDetectPolicy); // TODO: At the moment we are just merge everything, so in case of conflicts one will be taken

...
class DependencyResolverMain {
  ...
  getInstaller(options = {}) {
    ...
    return new (_dependencyInstaller().DependencyInstaller)(packageManager, this.aspectLoader, this.logger, options.rootDir, cacheRootDir, preInstallSubscribers, postInstallSubscribers, this.config.nodeLinker);
  }
  ...
  async getVersionResolver(options = {}) {
    ...
    const {
      networkConcurrency
    } = await this.getNetworkConfig(); // TODO: we should somehow pass the cache root dir to the package manager constructor
  ...
  async getNetworkConfig() {
    return _objectSpread(_objectSpread(_objectSpread({}, await this.getNetworkConfigFromGlobalConfig()), await this.getNetworkConfigFromPackageManager()), this.getNetworkConfigFromDepResolverConfig());
  }

  async getNetworkConfigFromGlobalConfig() {
    return _http().Http.getNetworkConfig();
  }

  getNetworkConfigFromDepResolverConfig() {
    return (0, _lodash().pick)(this.config, ['fetchTimeout', 'fetchRetries', 'fetchRetryFactor', 'fetchRetryMintimeout', 'fetchRetryMaxtimeout', 'maxSockets', 'networkConcurrency']);
  }

  async getNetworkConfigFromPackageManager() {
    const packageManager = this.getPackageManager();
    if (typeof (packageManager === null || packageManager === void 0 ? void 0 : packageManager.getNetworkConfig) !== 'function') return {};
    return packageManager.getNetworkConfig();
  }

  getPackageManager() {
    const packageManager = this.packageManagerSlot.get(this.config.packageManager);
    return packageManager !== null && packageManager !== void 0 ? packageManager : this.getSystemPackageManager();
  }
  ...
  async getMissingPeerDependencies(rootDir, rootPolicy, componentDirectoryMap, options) {
    ...
    if (packageManager !== null && packageManager !== void 0 && packageManager.getPeerDependencyIssues && typeof (packageManager === null || packageManager === void 0 ? void 0 : packageManager.getPeerDependencyIssues) === 'function') {
      peerDependencyIssues = await (packageManager === null || packageManager === void 0 ? void 0 : packageManager.getPeerDependencyIssues(rootDir, rootPolicy, componentDirectoryMap, options));
    } else {
      const systemPm = this.getSystemPackageManager();
      if (!systemPm.getPeerDependencyIssues) throw new Error('system package manager must implement `getPeerDependencyIssues()`');
      peerDependencyIssues = await (systemPm === null || systemPm === void 0 ? void 0 : systemPm.getPeerDependencyIssues(rootDir, rootPolicy, componentDirectoryMap, options));
    }
  ...
  async getRegistries() {
    ...
    const installFromBitDevRegistry = (_this$config$installF = this.config.installFromBitDevRegistry) !== null && _this$config$installF !== void 0 ? _this$config$installF : true; // Override default registry to use bit registry in case npmjs is the default - bit registry will proxy it
  ...
  static async provider([envs, loggerExt, configMain, aspectLoader, componentAspect, graphql, globalConfig], config, [rootPolicyRegistry, policiesRegistry, packageManagerSlot, dependencyFactorySlot, preInstallSlot, postInstallSlot]) {
    ...
    _dependencyResolver().DependencyResolver.registerHarmonyEnvPeersPolicyGetter(async configuredExtensions => {

...
async function index(component, scopePath) {
  // if (isWin) return Promise.resolve(component);
  try {
    indexInstance = await _serverlessIndex().default.initializeIndex(scopePath);
...
async function indexAll(path, components) {
  if (!components) return Promise.reject('The scope is empty');

  _logger().default.debug(`indexing all, scope path ${path}`);

  indexInstance = await _serverlessIndex().default.initializeIndex(path);

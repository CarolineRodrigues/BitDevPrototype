...
class BitLogger {
  ...
  async exitAfterFlush(code = 0, commandName, cliOutput = '') {
    ...
    const msg = isSuccess ? `[*] the command "${commandName}" has been completed successfully` : `[*] the command "${commandName}" has been terminated with an error code ${code}`; // this should have been helpful to not miss any log message when using `sync: false` in the

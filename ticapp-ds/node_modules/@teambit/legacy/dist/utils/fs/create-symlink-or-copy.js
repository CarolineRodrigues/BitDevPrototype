...
Original error: ${err}`);
  ...
  function link() {
    ...
    try {
      ...
      if (err.code === 'EXDEV') {
        _logger().default.trace(`createSymlinkOrCopy, link() found EXDEV error, trying fs native`); // this is docker, which for some weird reason, throw error: "EXDEV: cross-device link not permitted"
  ...
  function hardLinkOrJunctionByFsExtra(src) {
    try {
      ...
      if (err.code === 'EPERM') {
        _logger().default.trace(`createSymlinkOrCopy, hardLinkOrJunctionByFsExtra() using Junction option`); // it's a directory. use 'junction', it works on both Linux and Win
  ...
  function hardLinkOrJunctionByFsNative(src) {
    try {
      ...
      if (err.code === 'EPERM' || err.code === 'EXDEV') {
        _logger().default.trace(`createSymlinkOrCopy, hardLinkOrJunctionByFsNative() using Junction option`); // it's a directory. use 'junction', it works on both Linux and Win

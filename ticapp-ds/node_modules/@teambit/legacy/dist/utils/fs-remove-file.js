...
function _removeEmptyDir() {
  const data = _interopRequireDefault(require("./fs/remove-empty-dir"));
...
async function removeFile(path, propagateDirs = false) {
  try {
    await _fsExtra().default.unlink(path);
  } catch (err) {
    if (err.code === 'ENOENT') {
  ...
  await (0, _removeEmptyDir().default)(dir);
  return true;
}
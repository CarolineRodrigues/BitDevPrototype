...
class Component extends _objects().BitObject {
  ...
  async setDivergeData(repo, throws = true, fromCache = true) {
    if (!this.divergeData || !fromCache) {
      ...
      this.divergeData = await (0, _getDivergeData().getDivergeData)(repo, this, remoteHead, undefined, throws);
    }
  }
  ...
  async populateLocalAndRemoteHeads(repo, lane) {
    ...
    if (this.scope) {
      ...
      if (lane !== null && lane !== void 0 && lane.remoteLaneId) {
      ...
      this.remoteHead = await repo.remoteLanes.getRef(_laneId().RemoteLaneId.from(_constants().DEFAULT_LANE, this.scope), this.toBitId());
    }
  }
...
for a component "${this.id()}", versions: ${versions.join(', ')}`);
    ...
    try {
      ...
      loadedRefs.push(...loaded); // ignore missing artifacts when exporting old versions that were exported in the past and are now exported to a

...
class Http {
  ...
  static async getNetworkConfig() {
    const obj = await (0, _globalConfig().list)();
    return {
      fetchRetries: obj[_constants().CFG_FETCH_RETRIES],
      fetchRetryFactor: obj[_constants().CFG_FETCH_RETRY_FACTOR],
      fetchRetryMintimeout: obj[_constants().CFG_FETCH_RETRY_MINTIMEOUT],
      fetchRetryMaxtimeout: obj[_constants().CFG_FETCH_RETRY_MAXTIMEOUT],
      fetchTimeout: obj[_constants().CFG_FETCH_TIMEOUT],
      localAddress: obj[_constants().CFG_LOCAL_ADDRESS],
      maxSockets: obj[_constants().CFG_MAX_SOCKETS],
      networkConcurrency: obj[_constants().CFG_NETWORK_CONCURRENCY]
    };
  }
  ...
  async deprecateMany(ids) {
    throw new Error(`deprecation of a remote component has been disabled. deprecate locally with an updated version of bit and then tag and export`); // const DEPRECATE_COMPONENTS = gql`
  ...
  async undeprecateMany(ids) {
    throw new Error(`un-deprecation of a remote component has been disabled. undeprecate locally with an updated version of bit and then tag and export`); // const UNDEPRECATE_COMPONENTS = gql`
    //   mutation deprecate($bitIds: [String!]!) {
  ...
  static async connect(host, scopeName, localScopeName) {
    ...
    const agent = await Http.getAgent(host, _objectSpread(_objectSpread({}, proxyConfig), {}, {
...
async function getFetcherWithAgent(uri) {
  const proxyConfig = await Http.getProxyConfig();
  const networkConfig = await Http.getNetworkConfig();
  const agent = await Http.getAgent(uri, _objectSpread(_objectSpread({}, proxyConfig), {}, {
    localAddress: networkConfig.localAddress,
    maxSockets: networkConfig.maxSockets
  }));

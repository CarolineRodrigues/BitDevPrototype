  ...
  async execute(scope, options, authData) {
    ...
    if (ExportPersist.onPutHook) {
      ...
      ExportPersist.onPutHook(componentsIds, lanes, authData).catch(err => {
        _logger().default.error('fatal: onPutHook encountered an error (this error does not stop the process)', err); // let the process continue. we don't want to stop it when onPutHook failed.

...
async function removeLanes(consumer, lanes, remote, force) {
  if (remote) {
    const remoteLaneIds = lanes.map(lane => _laneId().RemoteLaneId.parse(lane));
  ...
  const workspaceLanes = lanes.map(lane => _workspaceLane().default.load(lane, consumer.scope.path));

 ...
 */
function groupDependencyList(dependenciesPaths, componentDir, bindingPrefix, isLegacyProject) {
  const resultGroups = new (_dependencyTreeType().DependenciesTreeItem)();

  const isPackage = str => str.includes('node_modules/');

  const isBitLegacyComponent = str => isLegacyProject && (str.includes(`node_modules/${bindingPrefix}`) || str.includes(`node_modules/${_constants().DEFAULT_BINDINGS_PREFIX}`));

  const isBitLegacyInsideHarmony = str => !isLegacyProject && (str.includes(`node_modules/${bindingPrefix}`) || str.includes(`node_modules/${_constants().DEFAULT_BINDINGS_PREFIX}`));
...
function MadgeTreeToDependenciesTree(tree, componentDir, bindingPrefix, isLegacyProject) {
  const result = {};
...
function mergeManuallyFoundPackagesToTree(foundPackages, missingGroups, tree) {
  if ((0, _isEmpty2().default)(foundPackages.components) && (0, _isEmpty2().default)(foundPackages.packages)) return; // Merge manually found packages (by groupMissing()) with the packages found by Madge (generate-tree-madge)
...
async function getDependencyTree({
  ...
  const tree = MadgeTreeToDependenciesTree(madgeTree, componentDir, bindingPrefix, isLegacyProject);
  const {
    missingGroups,
    foundPackages
  } = new (_missingHandler().MissingHandler)(skipped, componentDir, workspacePath, bindingPrefix, isLegacyProject).groupAndFindMissing();

...
class DependenciesLoader {
  ...
  async getDependenciesDataFromCacheIfPossible() {
    ...
    const componentConfigFilename = this.consumer.isLegacy ? _constants().PACKAGE_JSON : _constants().COMPONENT_CONFIG_FILE_NAME;

    const componentConfigPath = _path().default.join(this.consumer.getPath(), rootDir, componentConfigFilename);
  ...
  pushToDependencyResolverExtension(dataField, data, operation = 'add') {
    ...
    if (!ext) {
      extExistOnComponent = false; // Create new deps resolver extension entry to add to the component with data only

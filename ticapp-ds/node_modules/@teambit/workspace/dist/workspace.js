...
class Workspace {
  ...
  async get(componentId, forCapsule = false, legacyComponent, useCache = true, storeInCache = true) {
    ...
    const component = await this.componentLoader.get(componentId, forCapsule, legacyComponent, useCache, storeInCache); // When loading a component if it's an env make sure to load it as aspect as well
    ...
    if (tryLoadAsAspect && this.envs.isUsingEnvEnv(component) && !this.aspectLoader.isCoreAspect(component.id.toStringWithoutVersion()) && !this.aspectLoader.isAspectLoaded(component.id.toString()) && (await this.hasId(component.id)) // !config.extension(component.id.toStringWithoutVersion(), true)
      ...
      try {
        ...
        await this.loadAspects([component.id.toString()], undefined, component.id); // In most cases if the load self as aspect failed we don't care about it.
  ...
  async getCurrentRemoteLaneId() {
    ...
    const laneId = _laneId().RemoteLaneId.from(trackData.remoteLane, trackData.remoteScope);
  ...
  async idsByPattern(pattern, throwForNoMatch = true) {
    if (!pattern.includes('*') && !pattern.includes(',')) {
      ...
      if (throwForNoMatch) throw new (_bitError().BitError)(`unable to find "${pattern}" in the workspace`);
      return [];
    }

    const ids = await this.listIds();
    const patterns = pattern.split(',').map(p => p.trim()); // check also as legacyId.toString, as it doesn't have the defaultScope
  ...
  async getComponentsUsingEnv(env, throwIfNotFound = false) {
...
the following envs are used in this workspace: ${availableEnvs.join(', ')}`);
  ...
  async write(rootPath, component) {
    await Promise.all(component.filesystem.files.map(async file => {
      const pathToWrite = _path().default.join(this.path, rootPath, file.relative);
  ...
  async getSpecificComponentConfig(id, aspectId) {
    ...
    return (_this$bitMap$getBitma = this.bitMap.getBitmapEntry(id).config) === null || _this$bitMap$getBitma === void 0 ? void 0 : _this$bitMap$getBitma[aspectId];
  }
  /**
...
ids: ${ids.join(', ')}
needed-for: ${(neededFor === null || neededFor === void 0 ? void 0 : neededFor.toString()) || '<unknown>'}`);
    ...
    const scopeEnvsManifestsIds = scopeIdsGrouped.envs && scopeIdsGrouped.envs.length ? await this.scope.loadAspects(scopeIdsGrouped.envs, throwOnError) : [];
    const scopeOtherManifests = scopeIdsGrouped.other && scopeIdsGrouped.other.length ? await this.scope.getManifestsGraphRecursively(scopeIdsGrouped.other, (0, _lodash().compact)(workspaceManifestsIds), throwOnError, {
      packageManagerConfigRootDir: this.path
    }) : [];
  ...
  async buildOneGraphForComponents(ids, ignoreIds, shouldIgnoreFunc) {
    const graphFromFsBuilder = new (_buildGraphFromFs().GraphFromFsBuilder)(this, this.logger, ignoreIds, shouldIgnoreFunc);
  ...
  async install(packages, options) {
    ...
    if (options !== null && options !== void 0 && options.addMissingPeers) {
      ...
      const missingPeers = await this.dependencyResolver.getMissingPeerDependencies(this.path, mergedRootPolicy, compDirMap, pmInstallOptions);
  ...
  async _getComponentsWithDependencyPolicies() {
    ...
    (await Promise.all(allComponentIds.map(componentId => this.componentConfigFile(componentId)))).forEach((componentConfigFile, index) => {
...
your workspace.jsonc has this component-id set. you might want to remove/change it.`);
  ...
  async importObjects() {
    ...
    const importer = new (_importer().Importer)(this, this.dependencyResolver);

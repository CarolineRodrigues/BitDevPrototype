  ...
  async build(context) {
    ...
    const componentsResults = capsules.map(capsule => {
      ...
      const errors = srcFiles.map(srcFile => {
        try {
          const afterMdxCompile = (0, _mdxModules().compileSync)(srcFile.contents.toString('utf-8'));
          const afterBabelCompile = (0, _compilationModules().transpileFileContent)(afterMdxCompile.contents, {
          ...
          (0, _fsExtra().outputFileSync)((0, _path().join)(capsule.path, this.getDistPathBySrcPath(afterBabelCompile[0].outputPath)), afterBabelCompile[0].outputText);

          if (afterBabelCompile.length > 1) {
            (0, _fsExtra().outputFileSync)((0, _path().join)(capsule.path, this.distDir, afterBabelCompile[1].outputPath), afterBabelCompile[1].outputText);
          }

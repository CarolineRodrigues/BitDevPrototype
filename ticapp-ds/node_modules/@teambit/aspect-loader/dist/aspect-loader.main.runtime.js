...
class AspectLoaderMain {
  ...
  async getRuntimePath(component, modulePath, runtime) {
    ...
    try {
      ...
      this.logger.info(`got an error during get runtime path, probably the env is not loaded yet ${e}`); // TODO: we are manually adding the dist here and replace the file name to handle case when
  ...
  async doRequire(requireableExtension) {
    ...
    manifest.id = idStr; // It's important to clone deep the manifest here to prevent mutate dependencies of other manifests as they point to the same location in memory

...
class AspectMain {
  ...
  async setAspectsToComponents(pattern, aspectId, config = {}) {
  ...
  async updateAspectsToComponents(aspectId, pattern) {
    ...
    await this.workspace.bitMap.write();
    return (0, _lodash().compact)(updatedComponentIds);
  }
  /**
  ...
  static async provider([react, envs, builder, aspectLoader, compiler, babel, generator, workspace, cli]) {
    ...
    const compilerTasksOverride = react.overrideCompilerTasks([compiler.createTask('BabelCompiler', babelCompiler), compiler.createTask('TypescriptCompiler', tsCompiler)]);

...
class ElementsMain {
  ...
  async getElements(component) {
    const artifacts = await this.builder.getArtifactsVinylByExtension(component, _elements().ElementsAspect.id);
    if (!artifacts.length) return undefined;
    return new (_elementsArtifact().ElementsArtifact)(artifacts);
  }

  isElementsExist(component) {
    const artifacts = this.builder.getArtifactsByExtension(component, _elements().ElementsAspect.id);
    return !!(artifacts !== null && artifacts !== void 0 && artifacts.length);
  }

  async getElementUrl(component) {
    const artifacts = await this.getElements(component); // In case there are no elements return as undefined

    if (!artifacts) return undefined;
    if (artifacts !== null && artifacts !== void 0 && artifacts.isEmpty()) return undefined;
    const url = artifacts === null || artifacts === void 0 ? void 0 : artifacts.getMainElementsFileUrl(); // In case of public url (like cdn) return the public url

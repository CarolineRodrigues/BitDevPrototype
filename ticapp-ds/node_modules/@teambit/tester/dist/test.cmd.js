...
class TestCmd {
  constructor(tester, workspace, logger) {
    ...
    (0, _defineProperty2().default)(this, "options", [['w', 'watch', 'start the tester in watch mode.'], ['d', 'debug', 'start the tester in debug mode.'], ['a', 'all', 'test all components, not only new and modified'], ['', 'junit <filepath>', 'write tests results as JUnit XML format into the specified file path'], ['', 'coverage', 'show code coverage data'], ['e', 'env <id>', 'test only the given env'], ['s', 'scope <scope>', 'name of the scope to test'] // TODO: we need to reduce this redundant casting every time.
    ]);
  }
  ...
  async render([userPattern], {
    ...
    if (!components.length) {
      if (userPattern) throw new (_exceptions2().NoMatchingComponents)(userPattern);
      return {
        code: 0,

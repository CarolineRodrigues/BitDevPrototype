...
class TesterMain {
  ...
  async getTestsResults(component) {
    var _this$workspace, _componentStatus$modi;

    const entry = component.state.aspects.get(_tester().TesterAspect.id);
    const componentStatus = await ((_this$workspace = this.workspace) === null || _this$workspace === void 0 ? void 0 : _this$workspace.getComponentStatus(component));
    const data = this.builder.getDataByAspect(component, _tester().TesterAspect.id);

    if ((entry || data) && !(componentStatus !== null && componentStatus !== void 0 && (_componentStatus$modi = componentStatus.modifyInfo) !== null && _componentStatus$modi !== void 0 && _componentStatus$modi.hasModifiedFiles)) {
      return {
  ...
  static async provider([cli, envs, workspace, loggerAspect, graphql, ui, devFiles, builder], config) {
    ...
    const testerService = new (_tester2().TesterService)(workspace, config.patterns, logger, graphql.pubsub, devFiles);
    envs.registerService(testerService);
    devFiles.registerDevPattern(config.patterns);
    const tester = new TesterMain(graphql, envs, workspace, testerService, new (_tester3().TesterTask)(_tester().TesterAspect.id, devFiles), devFiles, builder);

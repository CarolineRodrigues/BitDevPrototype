...
class BuilderMain {
  constructor(envs, workspace, buildService, tagService, snapService, scope, isolator, aspectLoader, buildTaskSlot, tagTaskSlot, snapTaskSlot) {
    this.envs = envs;
  ...
  async getArtifactsVinylByExtensionAndTaskName(component, aspectName, taskName) {
  ...
  getArtifactsByExtensionAndTaskName(component, aspectName, taskName) {
    const artifacts = this.getArtifacts(component);
    return artifacts === null || artifacts === void 0 ? void 0 : artifacts.filter(artifact => artifact.task.id === aspectName && artifact.task.name === taskName);
  }
  ...
  getBuilderData(component) {
    var _component$state$aspe, _data$artifacts;

    const data = (_component$state$aspe = component.state.aspects.get(_builder().BuilderAspect.id)) === null || _component$state$aspe === void 0 ? void 0 : _component$state$aspe.data;
    if (!data) return undefined;
    (_data$artifacts = data.artifacts) === null || _data$artifacts === void 0 ? void 0 : _data$artifacts.forEach(artifact => {
      if (!(artifact.files instanceof _artifactFiles().ArtifactFiles)) {
        artifact.files = _artifactFiles().ArtifactFiles.fromObject(artifact.files);
      }
    });
    return data;
  }
  ...
  static async provider([cli, envs, workspace, scope, isolator, loggerExt, aspectLoader, graphql, generator], config, [buildTaskSlot, tagTaskSlot, snapTaskSlot]) {
    ...
    const builder = new BuilderMain(envs, workspace, buildService, tagService, snapService, scope, isolator, aspectLoader, buildTaskSlot, tagTaskSlot, snapTaskSlot);
...
(0, _defineProperty2().default)(BuilderMain, "dependencies", [_cli().CLIAspect, _envs().EnvsAspect, _workspace().WorkspaceAspect, _scope().ScopeAspect, _isolator().IsolatorAspect, _logger().LoggerAspect, _aspectLoader().AspectLoaderAspect, _graphql().GraphqlAspect, _generator().GeneratorAspect]);

...
class ArtifactsCmd {
  constructor(builder, scope) {
    ...
    this.scope = scope;
    (0, _defineProperty2().default)(this, "name", 'artifacts <pattern...>');
    (0, _defineProperty2().default)(this, "shortDescription", 'EXPERIMENTAL. list and download components artifacts');
...
`);
  ...
  async report([userPattern], artifactsOpts) {
    const artifactExtractor = new (_artifactExtractor().ArtifactExtractor)(this.scope, this.builder, userPattern, artifactsOpts);
    ...
    const outputArtifacts = (aspectId, artifactData) => {
      ...
      const artifactDataStr = artifactData.map(artifact => {
        const subTitle = _chalk().default.white(artifact.taskName);
